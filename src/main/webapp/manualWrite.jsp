...
<html class="yui3-js-enabled"><head><meta http-equiv="Content-Type" content="text/html; charset=UTF-8">
	...
	<script src="${pageContext.request.contextPath}/lib/jquery-3.2.1.min.js?v=<%=updateTime%>" type="text/javascript" charset="utf-8"></script>
...
// 				setInterval(resize, 2000);
			...
			function saveNotice(id){
				...
				var url = '${pageContext.request.contextPath}/manualInsert.do';
				if(id != ""){	//수정
					url = '${pageContext.request.contextPath}/manualUpdate.do';
				}
				...
				/* $.ajax({
	   				...
	   				success: function(data) {
	   					if(data.result == "1"){
	   						...
	   						window.location.href = "${pageContext.request.contextPath}/manual.do";
	   					}else{
			...
			function deleteNotice(id){
 				if(confirm("삭제 하시겠습니까?")){
 					...
 					$.ajax({
 		   				...
 		   				async: false,
 		   				url: '${pageContext.request.contextPath}/manualUpdate.do',
 		   				data: {	'title' : title , 'content': content , 'id' : id ,'hide' : '1'},
 		   				success: function(data) {
 		   					if(data.result == "1"){
 		   						alert("삭제 했습니다.");
 		   						window.location.href = "${pageContext.request.contextPath}/manual.do";
			...
			//공지사항 복원
			function restNotice(id){
				if(confirm("삭제된 내용을 복원 하시겠습니까?")){
					...
					$.ajax({
		   				...
		   				async: false,
		   				url: '${pageContext.request.contextPath}/manualUpdate.do',
		   				data: {	'title' : title , 'content': content , 'id' : id ,'hide' : '0'},
		   				success: function(data) {
		   					if(data.result == "1"){
		   						alert("복원 했습니다.");
		   						window.location.href = "${pageContext.request.contextPath}/manual.do";
			...
			function goNotice(){
				if(confirm("취소 하시면 내용이 삭제 됩니다.\r취소 하시겠습니까?")){
					window.location.href="${pageContext.request.contextPath}/manual.do?page=${param.page}&searchKeyword=${param.searchKeyword}";	
				}
			}
...
<!---------------------------------- 컨텐츠 영역 -------------------------------------->
<form name="uploadF" id="uploadF" method="post" action="/manualInsert.do" enctype="multipart/form-data">
    ...
    <table class="table">
        ...
        <tbody>
            <tr>
                <th>제목</th>
                <td>
                    <input type="text" name="noticeTitle" id="noticeTitle" class="w100" value="${data.notice.title }"/>
                </td>
            </tr>
            <tr>
                <th>내용</th>
                <td>
                    <textarea class="w100" name="ir1" id="ir1" rows="15"> ${data.notice.content }</textarea>
                </td>
            </tr>
            <tr>
                <th>첨부파일</th>
                <td>
                	<c:set var="fileName" value="${fn:split(data.notice.filepath, '/')}" />
                	<a href="${pageContext.request.contextPath}${data.notice.filepath}">${fileName[fn:length(fileName)-1]}</a>
                    <input type="file" name="file1" class="w100" id="file1">
                </td>
            </tr>
        </tbody>
    </table>
    <div class="btn-area">
        <input type="button" class="btn point" value="저장" onclick="saveNotice('${data.notice.id}');" />
        <input type="button" class="btn gray" value="취소" onclick="goNotice();" />
        <c:if test="${!empty data.notice.id}">
        	<input type="button" class="btn br" value="삭제" onclick="deleteNotice('${data.notice.id}');" />
        </c:if>
       	<c:if test="${!empty data.notice.id and data.notice.hide ne '0'}">
       		<input type="button" class="btn br" value="복원" onclick="restNotice('${data.notice.id}');"/>
       	</c:if>
    </div>
</div>
</form>
