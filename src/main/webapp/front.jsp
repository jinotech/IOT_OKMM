...
// 				setInterval(resize, 2000);
			...
			//나의 마인드맵 가져오기.
			function getMyMindMapList(){
 				           	
            	$.ajax({
    				type: 'post',
    				dataType: 'json',
    				async: false,
    				url: '${pageContext.request.contextPath}/mindmap/myMindMapList.do',
    				data: {'searchMapName' : '${param.searchMapName}'},
    				success: function(data) {
    					...
    					if(data.mapList != null){
    						...
    						$("#__mapList__").empty();
    						mapCount = data.mapList.length;
    						
    						for (var i = 0; i < data.mapList.length; i++) {
    							
    							var html = "";
    							
    							html += "<div class=\"mapdiv\">";
    							html += "    <div class=\"map-cont\">";
    							html += "       <img src=\"${pageContext.request.contextPath}/css/images/ex02.png\" alt=\"\" />";
    							html += "   </div>";
    							html += "    <p class=\"map-title\"><a href=\"/map/"+data.mapList[i].key+"\">"+data.mapList[i].name+"</a></p>";
    							html += "</div>";
    							
								$("#__mapList__").append(html);
    						}
    					}
...
<!---------------------------------- 컨텐츠 영역 -------------------------------------->
<!-- 슬라이더 플러그인 (http://kenwheeler.github.io/slick/) -->
...
<div class="contents">
    <h1 class="ok-title">
        ...
        <div class="global-search">
            <input type="text" name="searchMapName" id="searchMapName" value="${param.searchMapName}" class="search-txt" onkeypress="jsEnter(event)" />
            <input type="button" class="search-btn" value="검색" />
        </div>
    </h1>
    <div class="map-slider" id="__mapList__">
    </div>
    <h1 class="ok-title">
        마인드맵 사용가이드
        <a href="${pageContext.request.contextPath}/manual.do" class="more">더보기</a>
    </h1>
    <ul class="guide-list">
