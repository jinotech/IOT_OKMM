...
// 				setInterval(resize, 2000);
			...
			function getNoticeList(page){
            	...
            	$.ajax({
    				...
    				dataType: 'html',
    				async: false,
    				url: '${pageContext.request.contextPath}/noticeList.do',
    				data: {'keyword' : keyword , 'page' : page, 'bbs_gb' : '${param.bbs_gb}'},
    				success: function(data) {
    					
    					$("#__noticeList__").html(data);
			...
			function goNoticeView(id){
				...
				window.location.href = "${pageContext.request.contextPath}/noticeWrite.do?bbs_gb=${param.bbs_gb}&id="+id+"&page="+page+"&searchKeyword="+keyword;
				</c:if>
				<c:if test="${user.auth ne 'admin' }"> 
				window.location.href = "${pageContext.request.contextPath}/noticeView.do?bbs_gb=${param.bbs_gb}&id="+id+"&page="+page+"&searchKeyword="+keyword;
				</c:if>
			}
			
			$(document).ready(function(){
				setPageNumber('${param.page}');
			});
			...
			function goNoticeWrite(){
				window.location.href = "${pageContext.request.contextPath}/noticeWrite.do?bbs_gb=${param.bbs_gb}";
			}
...
<div class="contents">
    ...
    <div class="search-area">
        <input type="text" name="searchKeyword" id="searchKeyword" class="srch-txt" value="${param.searchKeyword }" />
        <input type="button" value="검색" class="srch-btn" onclick="getNoticeList('1');" >
    </div>
    <c:if test="${user.auth eq 'admin' }"> 
    <div class="btn-area text-right">
        <input type="button" value="등록" class="register-btn" onclick="goNoticeWrite();" />
    </div>
    </c:if>
    <div id="__noticeList__"></div>
</div>
<!---------------------------------- 컨텐츠 영역 -------------------------------------->
