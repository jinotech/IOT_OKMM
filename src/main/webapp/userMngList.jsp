...
// 				setInterval(resize, 2000);
			...
			function setPageNumber(page){
				...
				$("#page").val(page);
				window.location.href = "${pageContext.request.contextPath}/userMngList.do?page="+page;
			}
			//공지사항 목록
    ...
    <table class="table">
       ...
       <tbody>
       	...
       	<tr>
                ...
                <td>
                	<c:choose>
						<c:when test="${row.email eq 'kakao' }">kakao</c:when>    
						<c:when test="${fn:indexOf(row.firstname ,'@') != -1 }">Gmail</c:when>
                ...
                <td>
                	<c:choose>
                		<c:when test="${row.email eq 'kakako' }">-</c:when>
                		<c:otherwise>${row.email }</c:otherwise>
                	</c:choose>
                </td>
                <td>${row.created }</td>
