...
<head>
	...
	<script type="text/javascript">
		...
		function drawAllMapChart() {
			var data = new google.visualization.DataTable();
			
			data.addColumn('string', 'Map');
			data.addColumn('number', 'Count');
			
			var allmaps = parseInt(<c:out value="${data.countAllMaps}"/>);
			var guestmaps = parseInt(<c:out value="${data.countGuestMaps}"/>);
			
			data.addRow(["User Map", allmaps-guestmaps]);
			data.addRow(["Guest Map", guestmaps]);
			...
			var chart = new google.visualization.PieChart(document.getElementById('maps_chart'));
		...
		function loadDay(day, period) {
			$.ajax({
				...
				dataType: 'json',
				async: false,
				url: '${pageContext.request.contextPath}/mindmap/admin/stars/mapStats.do',
				data: {'func': 'countDay',
							'day': day,
							'period': period
							},
		...
		function drawTable(el, data) {
			var $tb = $('#'+el);
			$tb.empty();
			$tb.append('<tr><th>날짜</th><th>신규</th><th class="border_last">누적</th></tr>');
			var amount = 0;
			for(d in data) {
				amount += data[d].count;
...
<body>
	...
	<div class="cur_map">현재맵수: <a id="total_map"><c:out value="${data.countAllMaps}"/></a></div>
	...
	<div id="content_day">		
		...
		<table>
			<tr>
				<td><div>생성된 맵 수</div><div id="day_chart"></div></td>				
			</tr>
			<tr>
				<td><div>전체 맵 수</div><div id="day_chart_addition"></div></td>
			</tr>
		</table>
		</div>
		</div>
	...
	<div id="content_month">
		...
		<div class="small_box mapno">
		<div class="sml_box_title">월별 맵수</div>
		<div class="sml_box_con">
		<table>
			<tr>
				<td><div>생성된 맵 수</div><div id="month_chart"></div></td>				
			</tr>
			<tr>
				<td><div>전체 맵 수</div><div id="month_chart_addition"></div></td>
			</tr>
		</table>
		</div>
		</div>
	...
	<div id="content_year">
		...
		<div class="small_box mapno">
		<div class="sml_box_title">년별 맵수</div>
		<div class="sml_box_con">
		<table>
			<tr>
				
				<td><div>생성된 맵 수</div><div id="year_chart"></div></td>				
			</tr>
			<tr>
				<td><div>전체 맵 수</div><div id="year_chart_addition"></div></td>
			</tr>
		</table>
		</div>
		</div>
