...
// 				setInterval(resize, 2000);
			...
			function saveNotice(id){
				...
				var content_ko = document.getElementById("ir1").value;
				...
				var url = '${pageContext.request.contextPath}/noticeInsert.do';
				if(id != ""){	//수정
					url = '${pageContext.request.contextPath}/noticeUpdate.do';
				}
				...
				$.ajax({
	   				...
	   				async: false,
	   				url: url,
	   				data: {	'title' : title , 'content_ko': content_ko , 'id' : id , 'bbs_gb' : '${param.bbs_gb}'},
	   				success: function(data) {
	   					if(data.result == "1"){
	   						...
	   						window.location.href = "${pageContext.request.contextPath}/notice.do?bbs_gb=${param.bbs_gb}";
	   					}else{
			...
			function deleteNotice(id){
 				if(confirm("삭제 하시겠습니까?")){
 					...
 					$.ajax({
 		   				...
 		   				async: false,
 		   				url: '${pageContext.request.contextPath}/noticeUpdate.do',
 		   				data: {	'title' : title , 'content_ko': content_ko , 'id' : id ,'hide' : '1'},
 		   				success: function(data) {
 		   					if(data.result == "1"){
 		   						alert("삭제 했습니다.");
 		   						window.location.href = "${pageContext.request.contextPath}/notice.do?bbs_gb=${param.bbs_gb}";
			...
			function restNotice(id){
				if(confirm("삭제된 내용을 복원 하시겠습니까?")){
					...
					$.ajax({
		   				...
		   				async: false,
		   				url: '${pageContext.request.contextPath}/noticeUpdate.do',
		   				data: {	'title' : title , 'content_ko': content_ko , 'id' : id ,'hide' : '0'},
		   				success: function(data) {
		   					if(data.result == "1"){
		   						alert("복원 했습니다.");
		   						window.location.href = "${pageContext.request.contextPath}/notice.do?bbs_gb=${param.bbs_gb}";
			...
			function goNotice(){
				if(confirm("취소 하시면 내용이 삭제 됩니다.\r취소 하시겠습니까?")){
					window.location.href="${pageContext.request.contextPath}/notice.do?bbs_gb=${param.bbs_gb}&page=${param.page}&searchKeyword=${param.searchKeyword}";	
				}
			}
...
<div class="contents">
     <h1 class="ok-title">
        <c:choose>
     		<c:when test="${param.bbs_gb eq 'NTC' }">공지사항</c:when>
     		<c:when test="${param.bbs_gb eq 'QA' }">Q&A</c:when>
     		<c:when test="${param.bbs_gb eq 'FAQ' }">FAQ</c:when>
     	</c:choose> 등록
    </h1>
    <table class="table">
        <colgroup>
        ...
        <tbody>
            ...
            <tr>
                <th>내용</th>
                <td>
                    <textarea class="w100" name="ir1" id="ir1" rows="15"> ${data.notice.content_ko }</textarea>
                </td>
            </tr>
        </tbody>
    </table>
